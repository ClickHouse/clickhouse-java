...
import com.clickhouse.client.data.ClickHouseByteValue.UnsignedByteValue;
import com.clickhouse.client.data.ClickHouseIntegerValue.UnsignedIntegerValue;
import com.clickhouse.client.data.ClickHouseLongValue.UnsignedLongValue;
import com.clickhouse.client.data.ClickHouseShortValue.UnsignedShortValue;
...
public abstract class BaseDataProcessorTest {
    ...
    protected Object[][] getNestedTypesForRead() {
        ClickHouseConfig defaultConf = new ClickHouseConfig();
        ClickHouseConfig binStrConf = new ClickHouseConfig(
                Collections.singletonMap(ClickHouseClientOption.USE_BINARY_STRING, true));
        ClickHouseConfig widenUtConf = new ClickHouseConfig(
                Collections.singletonMap(ClickHouseClientOption.WIDEN_UNSIGNED_TYPES, true));
        ClickHouseConfig useObjsConf = new ClickHouseConfig(
                Collections.singletonMap(ClickHouseClientOption.USE_OBJECTS_IN_ARRAYS, true));
        ClickHouseConfig combinedConf = new ClickHouseConfig(widenUtConf, useObjsConf);
    ...
    public void testSerializeNestedTypes(ClickHouseConfig config, String typeName, String dataKey,
    ...
    public void testSerializeSimpleTypes(ClickHouseConfig config, String typeName, String dataKey,
            ClickHouseValue... values) throws IOException {
        if (values == null || values.length == 0) {
            Assert.fail("At least one value must be provided for testing");
        }
