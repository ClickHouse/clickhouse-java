...
- **[client-v2]** Fixed `Client.cancelTransportRequest(queryId)` being silently dropped when it landed between two
  ...
  request. A cancellation that lands before the operation sent its first request is no longer dropped either - it was
  possible to lose one with `useAsyncRequests(true)`, where the operation body runs on the shared executor: an operation
  is now registered for cancellation when it is started rather than when its first request is created, so it stops
  before any request reaches the server. (https://github.com/ClickHouse/clickhouse-java/issues/2989)
