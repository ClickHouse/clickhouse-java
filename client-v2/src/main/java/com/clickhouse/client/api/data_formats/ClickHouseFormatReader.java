...
/**
 * Common contract for row-by-row format readers regardless of the underlying
 * wire encoding. Implementations are produced for binary formats (see
 * {@link ClickHouseBinaryFormatReader}) and text formats (see
 * {@link ClickHouseTextFormatReader}).
 *
 * <p>The methods declared here describe row navigation, schema access, and
 * typed accessors by column name and 1-based column index. Concrete readers
 * may not support every accessor for every format; unsupported accessors are
 * expected to throw {@link UnsupportedOperationException}.</p>
 */
