...
public class SerializerUtils {
    ...
    /**
     * Converts a value read for a string-backed column into a {@link String} for POJO binding (any column the
     * reader may decode to a String, including top-level {@code String}/{@code FixedString} and wrappers such
     * as {@code SimpleAggregateFunction(String)}). With the binary-string feature enabled the reader produces a
     * {@link StringValue} holder (a read-time value holder, not a POJO field type); otherwise it produces a
     * plain {@link String}. A {@code null} value (e.g. a SQL {@code NULL}) is passed through. Used from the
     * bytecode generated by {@link #compilePOJOSetter(Method, ClickHouseColumn)}.
     *
    ...
    /**
     * Converts a value read for a string-backed column into the raw bytes for POJO binding. A {@link StringValue}
     * yields its preserved bytes (no lossy decoding); a plain {@link String} is encoded as UTF-8, matching the
     * historical behaviour; a value that is already a {@code byte[]} is passed through. A {@code null} value is
     * passed through. Used from the bytecode generated by {@link #compilePOJOSetter(Method, ClickHouseColumn)}.
     *
     * @param value value returned by {@code BinaryStreamReader.readValue}
        ...
        // setter setValue(Object obj, Object value) impl
        {
            ...
            if (targetType.isPrimitive() && BinaryStreamReader.isReadToPrimitive(column.getDataType())) {
                ...
                if (targetType == String.class) {
                    // The conversion is driven by the target field type, not the column data type: readValue may
                    // yield a StringValue holder for any string-backed column (top-level String/FixedString, but
                    // also wrappers such as SimpleAggregateFunction(String) or a Dynamic resolving to String) when
                    // binary-string support is enabled. Normalize it (or a plain String when the feature is off) to
                    // the String the setter expects. StringValue is a read-time holder, not a supported POJO field
                    // type, so it never reaches the setter directly.
                    ...
                    // Convert the StringValue (or plain String) to raw bytes for a byte[] field. A value that is
                    // already a byte[] (e.g. Array(UInt8)) passes through unchanged.
                    mv.visitMethodInsn(INVOKESTATIC,
                            Type.getInternalName(SerializerUtils.class),
