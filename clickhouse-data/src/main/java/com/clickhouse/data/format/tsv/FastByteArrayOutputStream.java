...
public final class FastByteArrayOutputStream extends OutputStream {
    ...
    public void copyFrom(FastByteArrayInputStream source, int offset, int count) {
        if (offset + count > source.getCount()) {
            throw new IndexOutOfBoundsException("Trying to copy data past the end of source" + ", source.size="
                    + source.getCount() + ", offset=" + offset + ", count=" + count);
        }
    ...
    /**
     * Creates InputStream using the same data that is written into this stream with
     * no copying in memory
     * 
