...
To configure initial JWT authentication on the client, use `useBearerTokenAuth(token)` (to add `Bearer_` prefix.
(see [HTTP iterface docs](/concepts/features/security/external-authenticators/jwt#http-interface) ) when building the `Client` instance:
