...
public class PreparedStatementImpl extends StatementImpl implements PreparedStatement, JdbcV2Wrapper {
    ...
    /**
     * Strategy interface for encoding objects into SQL string representations.
     */
    public interface ObjectEncodeStrategy {
        /**
         * Encodes an object into a SQL string representation.
         *
         * @param x the object to encode
         * @param clickHouseDataType optional target ClickHouse data type
         * @param scaleOrLength optional scale or length parameter for parameterized types
         * @return encoded SQL string representation
         * @throws SQLException if encoding fails
         */
        String encode(Object x, ClickHouseDataType clickHouseDataType, Integer scaleOrLength) throws SQLException;
    }

    /**
     * Strategy that uses CAST to convert objects to the target ClickHouse data type.
     */
    public class CastEncodeStrategy implements ObjectEncodeStrategy {
        @Override
        public String encode(Object x, ClickHouseDataType clickHouseDataType, Integer scaleOrLength) throws SQLException {
    ...
    /**
     * Strategy that performs direct object conversion without CAST.
     */
    public class ObjectConversionEncodeStrategy implements ObjectEncodeStrategy {
        @Override
        public String encode(Object x, ClickHouseDataType clickHouseDataType, Integer scaleOrLength) throws SQLException {
            // Direct conversion - ignore clickHouseDataType and scaleOrLength
            return encodeObject(x);
        }
    }

    private ObjectEncodeStrategy encodeStrategy = new ObjectConversionEncodeStrategy();

    /**
     * Sets the encoding strategy to use.
     *
     * @param strategy the encoding strategy
     */
    public void setEncodeStrategy(ObjectEncodeStrategy strategy) {
        this.encodeStrategy = strategy;
    }

    /**
     * Gets the current encoding strategy.
     *
     * @return the current encoding strategy
     */
    public ObjectEncodeStrategy getEncodeStrategy() {
    ...
    private String encodeObject(Object x, ClickHouseDataType clickHouseDataType, Integer scaleOrLength) throws SQLException {
        return encodeStrategy.encode(x, clickHouseDataType, scaleOrLength);
    }
}
