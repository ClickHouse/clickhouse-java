    ...
    public void shouldReturnColumnIndex() throws SQLException {
        ...
        try (Connection conn = getJdbcConnection()) {
            try (Statement stmt = conn.createStatement()) {
                try (ResultSet srcRs = stmt.executeQuery("SELECT * FROM rs_test_data ORDER BY id")) {
                    ResultSet rs = DetachedResultSet.createFromResultSet(srcRs, Collections.emptyList());
                    assertTrue(rs.next());
